...
if __name__ == "__main__":
    ...
    for nlab in dataName:
        ...
        gen = pd.read_csv('./data/relgen.csv').iloc[:,2:]
        gen.columns =col
        print(gen)
        nonavp2 = pd.read_csv('data/mergeremainnonavp/anti-nonavpremain.csv').iloc[:, 1:]
        nonavp2.columns = col
        print(nonavp2)
        print(nonavp2.shape)
        en_train2 = pd.concat([gen, nonavp2], axis=0)
        print(en_train2)
        gen_train2 = pd.concat([gen, nonavp2], axis=0).values
        gen_label2 = np.array([1 if i < gen.shape[0] else 0 for i in range(gen_train2.shape[0])])
        gen2,gen_label2 = read(gen_train2,gen_label2,flag=True,name='rel')
        print(gen2.shape)


        relpos = data.iloc[211:,:]
        relneg = data.iloc[:211,:]
        genpos2 = gen2.iloc[208:,:]
        genneg2 = gen2.iloc[:208,:]

        data1 = pd.concat([relpos,genpos2,relneg,genneg2],axis=0)
        print(data1.shape)

        lable1 = np.array([1 if i < data1.shape[0]/2 else 0 for i in range(data1.shape[0])])
        ...
        Train_sfs.to_csv('./Anti/Train/Gen/relbest.csv')
